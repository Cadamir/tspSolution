...
public class AntColonyOptimization2 {
    private final ArrayList<Node> stations;
    private final double[][] graph;
    private final double[][] trails;
    private final List<Ant2> ants = new ArrayList<>();
    ...
    public AntColonyOptimization2(String filename) {
        stations = new TspConverter().generateFromFile(filename);
        graph = Node.generateDistanceMatrix(stations).getMatrix();
        ...
        for (int i = 0; i < graph.length * Configuration.INSTANCE.antFactor; i++) {
            ants.add(new Ant2(graph.length));
        }
    }
...
//        stringBuilder.append("\nruntime          | ").append(System.currentTimeMillis() - runtimeStart).append(" ms");
    ...
    private int selectNextCity(Ant2 ant) {
    ...
    public void calculateProbabilities(Ant2 ant) {
